...
if [ $# -ne 7 ]
then
	echo "ERROR, expected arguments: $0 <clouds-dir> <init-poses> <original-pose-graph> <pairs> <cumulated-frames> <threshold-pts-dist> <min-frame-idx-diff>" >&2
	exit 1
fi
...
awk "\$2 < $THRESH" $PAIRS | cut -d: -f1 | sed 's/[()]//g' | tr "," " " | while read pair
do

...
DATA_BASEDIR=/media/matylda1/cnn_velodyne_data/hdf_data/j2_h2_forpaper

for d in $NETS/odometry_l335_b4_j2_h5* $NETS/odometry_l335_b4_j2_h7* 
do
	label=$(basename $d)
	hsize=$(grep -o '_h[0-9]*_' <<<$label | grep -o '[0-9]*')
	deploy_proto=$DEPLOYS/${label}_deploy.prototxt
	...
	echo "$best_iteration $LAST_ITERATION"
	...
	do
		...
		do
			...
			data_dir=$DATA_BASEDIR/$(cut -d"_" -f5- <<<$label | rev | cut -d"_" -f2- | rev)
			ls $data_dir/train/$seq* $data_dir/test/$seq* | sort --version-sort > test.files

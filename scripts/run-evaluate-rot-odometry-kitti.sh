...
ROT_Y_DATA=/media/files/cnn_velodyne_data/results/17-joined-Rt/13-odometry_l335_b1_shifts56_rcls_comp_fixshift-it200k-window
...
do
	...
	do 
		echo "Merging $i"
		~/workspace/but_velodyne_lib/scripts/merge_pose_files.py -t $TRANSL_DATA/$i -r $ROT_DATA/$i -ry $ROT_Y_DATA/$i > evaluation/results/ivelas/data/$i
	done
	cd evaluation

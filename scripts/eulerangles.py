...
def mat2eulerXYZ(M, cy_thresh=None):
    [z, y, x] = mat2eulerZYX(M, cy_thresh)
    return [x, y, z]

def euler2matXYZ(x=0, y=0, z=0):
...
def eulerXYZ2quat(x=0, y=0, z=0):
    return eulerZYX2quat(z, y, x)
...
def quat2eulerXYZ(q):
    ...
    return mat2eulerXYZ(nq.quat2mat(q))


def eulerZYX2angle_axis(z=0, y=0, x=0):
    ...
    # delayed import to avoid cyclic dependencies
    import nibabel.quaternions as nq
    return nq.quat2angle_axis(eulerZYX2quat(z, y, x))

def eulerXYZ2angle_axis(x=0, y=0, z=0):
    return eulerZYX2angle_axis(z, y, x)

def angle_axis2eulerZYX(theta, vector, is_normalized=False):
    ...
    return mat2eulerZYX(M)

def angle_axis2eulerXYZ(theta, vector, is_normalized=False):
    [z, y, x] = angle_axis2eulerZYX(theta, vector, is_normalized)
    return [x, y, z]

def angle_axis_denorm2eulerXYZ(vector, is_normalized=False):
    theta = np.linalg.norm(vector)
    [z, y, x] = angle_axis2eulerZYX(theta, vector/theta, is_normalized)
    return [x, y, z]

...
DATA_BASEDIR=/media/matylda1/kitti/dataset_odometry_velodyne_odom_cnn_data/joined2_hist3_batch4_skipped_and_succ_single_odom/

for d in $NETS/odometry_l335_b4_j2_h3_skip_and_succ_transl_olddata
do
	label=$(basename $d)
	...
	do
		...
		do
			...
			ls $DATA_BASEDIR/$seq* | sort --version-sort > test.files

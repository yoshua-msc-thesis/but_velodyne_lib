...
int main (int argc, char **argv) {
  ...
  for(vector<string>::iterator featFile = files_to_process.begin(); featFile < files_to_process.end(); featFile++) {
    ...
    PointCloud<PointXYZI>::Ptr classified_cloud(new PointCloud<PointXYZI>);
    PolarGridOfClouds::Ptr grid_summary = PolarGridOfClouds(cloud).summarize();
	...
	if(!cell.empty()) {
	  ...
	  classified_cloud->push_back(pt_classified);
	}
      }
    }

    KdTreeFLANN<PointXYZI> kdtree;
    kdtree.setInputCloud(classified_cloud);
    ...
    for(int i = 0; i < cloud.size(); i++) {
      ...
      vector<float> dist(1);
      PointXYZI search_pt;
      copyXYZ(cloud[i], search_pt);
      kdtree.nearestKSearch(search_pt, 1, id, dist);

      float prob = classified_cloud->at(id[0]).intensity;
      Visualizer3D::colorizeIntensity(prob, r, g, b);
      PointXYZRGB colored_pt(r, g, b);
      copyXYZ(cloud[i], colored_pt);
